...
function EditTodo({ todo, onUpdate, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await updateTodo(todo._id, formData);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
      <div className="flex justify-between items-center mb-3 sm:mb-4">
        <h1 className="text-lg sm:text-xl font-bold text-gray-800">Edit Todo</h1>
        <button
          onClick={onCancel}
      ...
      <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
        ...
        <div className="flex justify-end space-x-2 sm:space-x-3">
          ...
          >
            <FiSave className="mr-1" /> {isSubmitting ? 'Saving...' : 'Save'}
          </button>
        </div>
      </form>
    </div>
